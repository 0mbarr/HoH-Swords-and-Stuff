/*namespace Modifiers
{
	...
	class StackModifier : Modifier, IBuffWidgetInfo, IStackModifier
	{
		int m_timerC;
		int m_timer;
		...
		StackModifier(UnitPtr unit, SValue& params)
		{
			super(unit);
		...
		void AddStack(int num = 1)
		{
			...
			if (cast<Player>(m_owner) !is null)
				(Network::Message("PlayerStackModifierAdd") << m_ModifierId << num).SendToAll();
		}
		...
		bool TakeStack(int num = 1)
		{
			...
			if (cast<Player>(m_owner) !is null)
				(Network::Message("PlayerStackModifierTake") << m_ModifierId << num).SendToAll();
